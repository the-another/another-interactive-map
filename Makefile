...
DOCKER_RUN_ISOLATED = docker run --rm -w /app $(DOCKER_IMAGE)
...
# Run PHPCS linter (runs in Docker, isolated - copies files in)
lint:
	@docker build --quiet -t $(DOCKER_IMAGE) .
	@docker create --name aucteeno-lint-tmp $(DOCKER_IMAGE) sh -c "composer install && ./vendor/bin/phpcs --standard=.phpcs.xml.dist"
	@docker cp . aucteeno-lint-tmp:/app
	@docker start -a aucteeno-lint-tmp || true
	@docker rm aucteeno-lint-tmp

# Format code using PHPCBF (runs in Docker)
format: docker-build
	$(DOCKER_RUN) composer phpcbf

# Run PHPUnit tests (runs in Docker, isolated - copies files in)
test:
	@docker build --quiet -t $(DOCKER_IMAGE) .
	@docker create --name aucteeno-test-tmp $(DOCKER_IMAGE) sh -c "composer install && php ./vendor/bin/phpunit"
	@docker cp . aucteeno-test-tmp:/app
	@docker start -a aucteeno-test-tmp || true
	@docker rm aucteeno-test-tmp
